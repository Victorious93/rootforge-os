...
- **Unified CLI:** `rootforge` (`/usr/local/bin/rootforge`) is a thin wrapper around a Python package at `/usr/local/lib/rootforge/core/`. Today: `rootforge doctor` (host tooling, disk space, `~/rootforge` writability, attached devices, optional AI tooling — with `--json`, `--quiet`, `--strict`), `rootforge devices` (adb + fastboot in one list, `-l` for codename/slot/lock state, `--json`), `rootforge module scaffold|lint|build` (P2 item 10), `rootforge flash boot` / `rootforge backup create|list|restore`, `rootforge ota extract|inspect` (P2 item 12), `rootforge boot patch|flash-last` (part of P2 item 11), and `rootforge avd create|boot|list` (P2 item 13). The `module` group is the first slice of the migration in `docs/IMPLEMENTATION_PLAN.md`: it **wraps** `new_module_scaffold.sh`, `lint_module.sh` and `build_magisk_module.sh` rather than reimplementing them — their device behaviour is proven — and moves only the argument handling into Python. That is not cosmetic. Every bug sweep over these scripts re-found the same four shell-specific failures: an unguarded `"$2"` (raw "unbound variable"), a `case` with no catch-all (a typo'd flag runs with defaults, silently), an exit code that reports success after total failure, and a `pipefail` abort that kills the script before its own error message. `argparse` gives all four for free, once, instead of needing a hand-written guard in every script. The `flash`/`backup` groups were ported next on purpose: they take the most arguments and they are destructive, so a mis-parsed one costs a device rather than a retry. `argparse` rejects a missing image, a zero-byte image, an unsupported partition, an abbreviated flag, or a codename/timestamp that would escape `~/rootforge/devices/` **before** `fastboot` is invoked at all. The `ota` group is there for the same reason in a different shape: an optional positional output directory next to a flag is what let `extract_ota.sh ota.zip --partitions boot` read the flag as the directory name and extract into one literally called `--partitions`, with the partition list silently left at its default. As a flag, that ambiguity does not exist. The `boot` group covers only the part of P2 item 11 that wraps proven code. That item also lists `unpack`, `repack` and `verify` as a unified front end over magiskboot, avbtool and mkbootimg; those do not exist as scripts yet, so building them would be new boot-image handling — and new boot-image handling cannot be verified without real images and a device to flash them to. Existing scripts keep working standalone and are unaffected — they carry the same guards themselves for anyone calling them directly.
